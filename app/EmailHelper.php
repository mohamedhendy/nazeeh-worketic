...
class EmailHelper extends Model
{
    ...
    public static function getEmailHeader()
    {
        ...
        $email_banner = !empty($setting) ? url('uploads/settings/email/'.$setting[0]['email_banner']): '';
        ?>
    ...
    public static function getEmailFooter()
    {
        ...
        $copyright = !empty($setting) ? $setting['copyright'] : 'Copyright Worketic All Rights Reserved';
        ?>
        </div>
        </div>
        </div>
    ...
    public static function getSiteTitle()
    {
        ...
        $title = !empty($settings[0]['title']) ? $settings[0]['title'] : 'Worketic';
        return $title;
    }
    ...
    {
        $setting = SiteManagement::getMetaValue('email_data');
        $email = !empty($setting[0]['from_email']) ? $setting[0]['from_email'] : 'info@amentotech.com';
        return $email;
    }
    ...
    {
        $setting = SiteManagement::getMetaValue('email_data');
        $email_id = !empty($setting[0]['from_email_id']) ? $setting[0]['from_email_id'] : 'info@amentotech.com';
        return $email_id;
    }
    ...
    public static function getSiteLogo()
    {
        ...
        $logo = !empty($setting[0]['logo']) ? $setting[0]['logo'] : '';
    ...
    public static function getSignature()
    {
        ob_start();
        $setting = SiteManagement::getMetaValue('email_data');
        $sender_name = !empty($setting[0]['sender_name']) ? $setting[0]['sender_name'] : 'Worketic';
        $sender_tagline = !empty($setting[0]['sender_tagline']) ? $setting[0]['sender_tagline'] : 'Worketic A Better Workplace for Employers and Freelancers';
        $sender_url = !empty($setting[0]['sender_url']) ? $setting[0]['sender_url'] : 'URL';
        ?>
