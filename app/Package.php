    ...
    */
    public function savePackage($request)
    {
        if (!empty($request)) {
            ...
            $this->role_id = filter_var($request['roles'][0], FILTER_SANITIZE_STRING);
            if ($request->roles == '2') {
    ...
    {
        if (!empty($request)) {
            $package = self::where('slug', $slug)->first();
            ...
            $package->role_id = filter_var($request['roles'][0], FILTER_SANITIZE_STRING);
            if ($request->roles === '2') {
